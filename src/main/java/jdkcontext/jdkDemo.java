...
//        c="a";  //报错
    ...
    @Test
    public void IntegerMax() throws InterruptedException {
        int count=1000000;
        int gva=count/5;
        System.out.println(gva);
        int start=0;
        int end=0;
        for (int i=1;i<6;i++){
            start=end+1;
            end=gva*i;
           new Thread(new Writer(start,end)).start();
        }
    ...
    class Writer implements  Runnable{
        ...
        public void run() {
            try {
                ...
                FileOutputStream outputStream = new FileOutputStream(new File(Thread.currentThread().getName() + ".txt"));
                for(;start<end;start++){
                    sb.append(start);
                    if(sb.length()>100000){
